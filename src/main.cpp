...
// Copyright (c) 2014 EdwardSnowdenCoin developers
...
const string strMessageMagic = "EdwardSnowdenCoin Signed Message:\n";
...
static const int64 nStartSubsidy = 10000 * COIN; //100 * COIN;
static const int64 nMinSubsidy = 1 * COIN; //100 * COIN;
...
static const int64 nTargetSpacing = 30; // 7 minutes
...
{
        static const int64 BlocksTargetSpacing = 1 * 60; // 30 seconds
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EdwardSnowdenCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EdwardSnowdenCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("EdwardSnowdenCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running EdwardSnowdenCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EdwardSnowdenCoinMiner terminated\n");
        throw;
    }
}
